/**
 * Detects external plugins that may conflict with oh-my-opencode-beads features.
...
/**
 * Known notification plugins that conflict with oh-my-opencode-beads's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[oh-my-opencode-beads] External notification plugin detected: ${pluginName}

Both oh-my-opencode-beads and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   oh-my-opencode-beads's session-notification has been auto-disabled.

   To use oh-my-opencode-beads's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in oh-my-opencode-beads.json`
}
