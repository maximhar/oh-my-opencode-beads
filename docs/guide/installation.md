...
3. **Need precision?** Press **Tab** to enter Prometheus (Planner) mode, create a work plan through an interview process, then run `/start-work` to hand off to Atlas. Execution is beads-driven (`bd ready` → `bd update --status in_progress` → `bd close`).
